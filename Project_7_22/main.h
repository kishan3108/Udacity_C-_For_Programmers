...
float calcPaintCost(float PAINTCOST, float wallSurfaceArea);
float calcWallPaperCost(float WALLPAPERCOST, float wallSurfaceArea);
void printWallCostComparisons(float paintCost, float wallpaperCost);
float calcFloorArea(float roomWidth, float roomDepth);
float calcCarpetCost(float CARPETCOST, float floorArea);
float calcTileCost(float TILECOST, float floorArea);
float calcWoodFloor(float WOODCOST, float floorArea);
